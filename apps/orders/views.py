...
def order_details(order_id):
    ...
    if order.user_id is user.id or user.is_admin():
        ...
        return get_error_response(
            "Access denied, this does not belong to you", status_code=401
        )
...
def create_order():
    ...
    if address_id is not None:
        ...
        if address is None:
            return get_error_response(
                "Permission Denied, you can not use this address", 401
            )
    else:
    ...
    if len(products) != len(cart_items):
        return get_error_response(
            "Error, make sure all products you want to order are still available"
        )
    ...
    return get_success_response(
        ...
        data=order.get_summary(include_order_items=True),
        status_code=200,
    )
