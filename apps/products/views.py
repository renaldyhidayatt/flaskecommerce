...
def create():
    if current_user.is_not_admin():
        return jsonify(
            get_error_response("Permission denied, you must be admin", status_code=401)
        )
    ...
    for header_key in list(request.form.keys()):
        if "tags[" in header_key:
            ...
            tags.append(
                get_or_create(db.session, Tag, {"description": description}, name=name)[
                    0
                ]
            )
        ...
        if header_key.startswith("categories["):
            ...
            if len(result.groups()) == 1:
                ...
                categories.append(
                    get_or_create(
                        db.session, Category, {"description": description}, name=name
                    )[0]
                )
...
def update(product_slug):
    ...
    if not (name and description and price and stock and price):
        return jsonify(
            get_error_response("You must provide a name, description, stock and price")
        )
