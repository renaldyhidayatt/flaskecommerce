...
def list_comments(product_slug):
    ...
    product_id = (
        Product.query.filter_by(slug=product_slug).with_entities("id").first()[0]
    )
    comments = (
...
def update_comment(comment_id):
    ...
    if current_user.is_admin() or comment.user_id == current_user.id:
        ...
        return get_error_response(
            "Permission denied, you can not update this comment", status_code=401
        )
...
def destroy_comment(comment_id):
    ...
    if current_user.is_admin() or comment.user_id == current_user.id:
        ...
        return get_error_response(
            "Permission denied, you can not delete this comment", status_code=401
        )
