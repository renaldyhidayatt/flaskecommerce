...
class PageSerializer(object):
    def __init__(self, pagination_obj, **kwargs):
        ...
        self.data = {}
        self.items = [
            resource.get_summary(**kwargs) for resource in pagination_obj.items
        ]
        ...
        self.data["next_page_number"] = (
            pagination_obj.next_num or self.data["current_page_number"]
        )
...
def get_success_response(messages, data=None, status_code=200):
    ...
    response = {"success": True, "full_messages": msgs}
...
def get_error_response(messages, status_code=500):
    ...
    return jsonify({"success": False, "full_messages": msgs}), status_code
